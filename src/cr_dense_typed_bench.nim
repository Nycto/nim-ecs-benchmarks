...
const
  PosVelMask = maskOf(Position, Velocity)
  PosVelAccMask = maskOf(Position, Velocity, Acceleration)
...
proc runDenseBenchmarks() =
  ...
  suite.add benchmarkWithSetup(
    ...
    (
      var w = setupWorldNoEnt()
      var ents = w.createTEntities(ENTITY_COUNT, Position, Velocity)
      var ents2: seq[TDHandle[PosVelAccMask]]
      for e in ents:
  ...
  suite.add benchmarkWithSetup(
    ...
    (
      var w = setupWorldNoEnt()
      var ents = w.createTEntities(ENTITY_COUNT, Position)
      var ents2: seq[TDHandle[PosVelMask]]
